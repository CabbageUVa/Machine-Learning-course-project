...
class SvmIncomeClassifier:
        ...
        for row in data:
            ...
            for i in range(0, row.size - 1):
                if col_names_x[i] in numerical_cols:
                    num_temp.append(row[i])
                elif col_names_x[i] in categorical_cols:
                    cat_temp.append(row[i])
    ...
    def pre_cat(self, cat_data, is_train):
        tags = []
        tags.append(['?', 'Private', 'Self-emp-not-inc', 'Self-emp-inc', 'Federal-gov', 'Local-gov', 'State-gov', 'Without-pay', 'Never-worked'])
        tags.append(['?', 'Bachelors', 'Some-college', '11th', 'HS-grad', 'Prof-school', 'Assoc-acdm', 'Assoc-voc', '9th',
					'7th-8th', '12th', 'Masters', '1st-4th', '10th', 'Doctorate', '5th-6th', 'Preschool'])
        tags.append(['?', 'Married-civ-spouse', 'Divorced', 'Never-married', 'Separated', 'Widowed', 'Married-spouse-absent', 'Married-AF-spouse'])
        tags.append(['?', 'Tech-support', 'Craft-repair', 'Other-service', 'Sales', 'Exec-managerial', 'Prof-specialty', 'Handlers-cleaners',
    ...
    def splitFolds(self, x_train, y_train, fold_num, num_test):
        trainSetX = []
        trainSetY = []
        testSetX  = []
        testSetY  = []
		
        col_size = x_train.size/x_train[0].size
		
        for i in range(col_size):
        	if i > (num_test*col_size/fold_num) and i < ((num_test+1)*col_size/fold_num):
                        testSetX.append(x_train[i])
                        testSetY.append(y_train[i])
			
                else:
                        trainSetX.append(x_train[i])
                        trainSetY.append(y_train[i])
    ...
    def train_and_select_model(self, training_csv):
        ...
        for num_model in range(len(param_set)):
            ...
            for num_test in range(fold_num):
                trainX, trainY, testX, testY = self.splitFolds(x_train, y_train, fold_num, num_test)
            ...
            print("\n\n\nThe score is %.2f. for Kernel: " % temp_score) + param.get('kernel')
	    print("\n\n\n")
